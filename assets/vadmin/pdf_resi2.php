...
function __construct($orientation='P', $unit='cm', $size='legal')
 ...
 /* setting zona waktu */
	...
	$pdf->MultiCell(7.5,0.4,''.$d->penerima. "	
".$d->dept2. " 
".$d->alamat. " 
".$this->app_model->find_kec($d->kec_id).", ".$this->app_model->find_kokab($d->kokab_id).", ".$this->app_model->find_prov($d->prov_id),'L','L');
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->SetXY(15,3.6);
	$pdf->Cell(2.9,0.5,'DFOD / COD','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->harga1),'LBR',0,'C');
	$pdf->Cell(2.0,0.5,''.number_format($d->harga1,0),'LBR',0,'R');
	$pdf->Ln(0.5);
	$pdf->SetFont('Helvetica','I',7); //set ulang font
	$pdf->Cell(6.5,0.6,'PENGIRIM','LR',0,'L');
	// $pdf->Cell(7.5,0.6,'--','LR',0,'L');
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->SetXY(15,4.1);
	$pdf->Cell(2.9,0.5,'Biaya Kirim','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->harga2),'LBR',0,'C');
	$pdf->Cell(2.0,0.5,''.number_format($d->harga2,0),'LBR',0,'R');
	$pdf->Ln(0.5,0.1);
	$pdf->SetFont('Helvetica','',8); //set ulang font
...
'.$this->app_model->find_kec($d->kec).", ".$this->app_model->find_kokab($d->kokab).", ".$this->app_model->find_prov($d->prov),'LR','L');
	...
	$pdf->SetXY(7.5,4.5);
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->MultiCell(7.5,0.5,'','LR','L');
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->SetXY(15,4.6);
	$pdf->Cell(2.9,0.5,'Biaya Kirim EXPRES','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->harga3),'LBR',0,'C');
	$pdf->Cell(2.0,0.5,''.number_format($d->harga3,0),'LBR',0,'R');
		$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->SetXY(15,5.1);
	$pdf->Cell(2.9,0.5,'Biaya Asuransi','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->harga4),'LBR',0,'C');
	$pdf->Cell(2.0,0.5,''.number_format($d->harga3,0),'LBR',0,'R');
	$pdf->Ln(0.1);
	$pdf->SetFont('Helvetica','',8); //set ulang font
	...
	$pdf->SetXY(15,5.6);
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(2.9,0.5,'Biaya Packaging','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->harga5),'LBR',0,'C');
	$pdf->Cell(2.0,0.5,''.number_format($d->harga4,0),'LBR',0,'R');
	$pdf->SetXY(15,6.1);
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(2.9,0.5,'Diskon','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->diskon),'LBR',0,'C');
	$pdf->Cell(2.0,0.5,''.number_format($d->diskon,0).'%','LBR',0,'R');	//garis baru
	$pdf->Ln();
	$pdf->Cell(6.5,0.4,'','LR',0,'L');
	$pdf->Cell(7.5,0.4,'','LR',0,'L');
	$pdf->SetFont('Helvetica','I',8); 
	$pdf->Cell(5.4,0.4,'','R',0,'L');
	$pdf->Ln();
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(6.5,0.4,'Telp.  '.$this->app_model->find_telp_pel($d->pel_id),'LBR',0,'LR');
		$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(7.5,0.4,'Telp.  '.$d->telp,'LB',0,'');
    $pdf->SetFont('Helvetica','B',9); //set ulang font
	...
	$pdf->Cell(12.9,0.4,'Terbilang','LR',0,'L');
	$pdf->Ln();
	...
	$pdf->Cell(6.5,0.7,''.$d->catatan,'LBR',0,'L');
	$pdf->Cell(12.9,0.7,'#'.ucwords(strtolower($this->app_model->bilang($d->totalbayar).'rupiah')).' #','LBR',0,'C');
	
	$pdf->Ln();
	$pdf->SetFont('Helvetica','B',8); //set ulang font
	$pdf->Cell(6.5,0.4,'ISI TIDAK DIPERIKSA DAN TIDAK DIKETAHUI','LBR',0,'L');
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(3.5,0.4,'Pengirim','LR',0,'C');
		$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(3.5,0.4,'Diterima Oleh','LR',0,'C');
	$pdf->Cell(3.5,0.4,'Operator','LR',0,'C');
	$pdf->Cell(2.4,0.4,'','LR',0,'C');
	$pdf->Ln();
	$pdf->SetFont('Helvetica','',7); //set ulang font
	//$pdf->Cell(6.5,0.6,''.$vol.'','LBR',0,'C');
	$pdf->Cell(6.5,0.6,'KETERANGAN ISI','LR',0,'C');
	...
	$pdf->Cell(6.5,0.4,''.$d->deskripsi,'LR',0,'C');
	...
	$pdf->Ln();
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(6.5,0.8,'','LRB',0,'L');
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(3.5,0.8,'Nama / ttd','LRB',0,'C');
	$pdf->Cell(3.5,0.8,'Nama / ttd / Cap','LRB',0,'C');
	$pdf->Cell(3.5,0.8,''.$this->app_model->find_nama_admin($d->user_id),'RLB',0,'C');
	$pdf->Cell(2.4,0.8,'','RLB',0,'C');
	$pdf->Ln(0.8);
...
'.$this->app_model->find_kec($d->kec).", ".$this->app_model->find_kokab($d->kokab).", ".$this->app_model->find_prov($d->prov),'LR','L');
	...
	$pdf->Cell(2.9,0.5,'Biaya Packaging','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->harga5),'LBR',0,'C');
	$pdf->Cell(2.0,0.5,''.number_format($d->harga5,0),'LBR',0,'R');	//garis baru
		$pdf->SetXY(15,17.1);
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(2.9,0.5,'Diskon','LBR',0,'L');
	$pdf->Cell(0.5,0.5,''.$this->app_model->ceklist_harga($d->diskon),'LBR',0,'C');
...
'.$this->app_model->find_kec($d->kec).", ".$this->app_model->find_kokab($d->kokab).", ".$this->app_model->find_prov($d->prov),'LR','L');
	...
	$pdf->Cell(6.5,0.8,'','LRB',0,'L');
	$pdf->SetFont('Helvetica','',8); //set ulang font
	$pdf->Cell(3.5,0.8,'Nama / ttd','LRB',0,'L');
	$pdf->Cell(3.5,0.8,'Nama / ttd / Cap','LRB',0,'L');
	$pdf->Cell(3.5,0.8,''.$this->app_model->find_nama_admin($d->user_id),'RLB',0,'C');
	$pdf->Cell(2.4,0.8,'','RLB',0,'C');
	$pdf->Ln(0.8);
	$pdf->SetFont('Helvetica','I',5.7); //set ulang font
