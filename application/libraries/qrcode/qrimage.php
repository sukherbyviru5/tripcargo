...
class QRimage {
    ...
    private static function image($frame, $pixelPerPoint = 4, $outerFrame = 4, $back_color = 0xFFFFFF, $fore_color = 0x000000)
    {
        ...
        // Validasi frame
        if ($h <= 0 || $w <= 0) {
            throw new Exception('Frame QR code tidak valid: lebar atau tinggi kosong');
        }
        ...
        if ($imgW <= 0 || $imgH <= 0) {
            throw new Exception('Ukuran gambar tidak valid: lebar atau tinggi <= 0');
        }
        ...
        // Konversi $pixelPerPoint ke integer untuk menghindari TypeError
        $pixelPerPoint = (int) $pixelPerPoint;

        // Validasi ukuran target gambar
        $targetW = $imgW * $pixelPerPoint;
        $targetH = $imgH * $pixelPerPoint;
        if ($targetW <= 0 || $targetH <= 0) {
            ImageDestroy($base_image);
            throw new Exception('Ukuran target gambar tidak valid: lebar atau tinggi <= 0');
        }
